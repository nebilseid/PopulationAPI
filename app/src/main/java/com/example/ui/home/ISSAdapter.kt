...
class ISSAdapter : RecyclerView.Adapter<ISSAdapter.ISSViewHolder>() {
    ...
    class ISSViewHolder(val view: View) : RecyclerView.ViewHolder(view) {
        ...
        fun bind(response: Response) {
            view.tvCountry.text = response.country.toString()
            view.tvAge.text = response.age.toString()
            view.tvFemales.text = response.females.toString()
            view.tvMales.text = response.males.toString()
            view.tvTotal.text = response.total.toString()
            view.tvYear.text = response.year.toString()
        }
